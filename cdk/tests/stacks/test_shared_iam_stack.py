...
def test_writer_role_name_is_deterministic():
    """Writer role name must be predictable for cross-account ARN construction."""
    _, template = _synth()
    writer = _find_role(template, "mmc:role", "baseline-writer")["resource"]
    assert writer["Properties"]["RoleName"] == "mmc-test-baseline-writer"


def test_writer_role_name_reflects_environment():
    """Writer role name embeds environment tag so prod/test never collide."""
    props = _valid_props(environment="prod")
    _, template = _synth(props)
    writer = _find_role(template, "mmc:role", "baseline-writer")["resource"]
    assert writer["Properties"]["RoleName"] == "mmc-prod-baseline-writer"


def test_reader_role_names_stay_autogenerated():
    """Readers scale N-per-artifact-stack, so keep CDK auto-naming."""
    _, template = _synth()
    for logical_id, role in template.find_resources("AWS::IAM::Role").items():
        if logical_id.startswith("BaselineReader"):
