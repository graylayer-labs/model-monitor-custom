...
def test_noop_shadow_analyser_returns_canned_output():
    analyser = NoopShadowAnalyser()
    ...
    assert output.payload == {"note": "NoopShadowAnalyser — real Shadow math in Phase 5"}
...
def test_noop_shadow_analyser_does_no_boto3_calls():
...
def test_noop_shadow_analyser_module_has_no_banned_imports():
    import analyser_shadow.analyser as mod
    ...
    for name in list(sys.modules):
        if name.startswith("analyser_shadow") and "boto3" in getattr(sys.modules[name], "__dict__", {}):
