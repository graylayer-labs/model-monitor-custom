...
def test_noop_mq_analyser_returns_canned_output():
    analyser = NoopMqAnalyser()
    ...
    assert output.payload == {"note": "NoopMqAnalyser — real MQ math in Phase 5"}
...
def test_noop_mq_analyser_does_no_boto3_calls():
...
def test_noop_mq_analyser_module_has_no_banned_imports():
    ...
    for name in list(sys.modules):
        if name.startswith("analyser_mq") and "boto3" in getattr(sys.modules[name], "__dict__", {}):
