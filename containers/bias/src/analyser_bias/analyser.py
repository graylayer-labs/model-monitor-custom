"""Phase 2 skeleton bias analyser.

`NoopBiasAnalyser` returns a canned :class:`AnalyserOutput` so we can
prove the base+analyser plumbing (env contract, IO, DDB, CW, provenance)
without shipping bias math. Phase 3 replaces this class with a
``smclarify``-backed implementation (DPL, CDDL, ...).
"""
...
class NoopBiasAnalyser:
    """Phase 2 no-op bias analyser.
    ...
    no bias math; Phase 3 supplies the real smclarify wrapper.
    """
    ...
    def compute(self, inputs: AnalyserInputs, config: dict[str, Any]) -> AnalyserOutput:  # noqa: PLR6301
        ...
        return AnalyserOutput(
            ...
            violation_count=0,
            analyser_metrics={"NoopSignal": 0.0},
            run_started_at=started,
            run_ended_at=ended,
            payload={"note": "NoopBiasAnalyser — real math in Phase 3"},
        )
