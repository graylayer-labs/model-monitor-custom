...
- [x] **2.3 — `InferenceMonitorStack` (Lambda compute backend).** in `cdk/src/model_monitor_cdk/stacks/inference_monitor_stack.py`. EventBridge Scheduler cron → SFN Standard → Lambda Parallel (5 branches) → CW + DDB. DDB Streams → EventBridge Pipes (alert + archive fan-out). Per-branch Retry/Catch. **Lambda as default** with config-driven ECS fallback for real AWS parity later. Tests for state-machine shape + Lambda resource provisioning.
- [x] **2.3.1 — `containers/base/` Lambda handler.** New `lambda_handler.py` bridging SFN payload (lowercase keys) to env var contract (uppercase). Reuses existing `_run_analyser`, `_emit_success`, `_emit_failure` from entrypoint. Dockerfile.lambda variants (base + 5 analysers).
- [x] **2.3.2 — Config toggle for compute backend.** `cdk/src/model_monitor_cdk/config.py`: `compute_backend: Literal["lambda", "ecs"] = "lambda"`. Dispatch in stack construction for `_build_lambda_functions` vs `_build_task_definitions`. ECS tests verify regression.
...
- [x] **2.8 — LocalStack end-to-end test harness.** `docker-compose.localstack.yml`, `tests/e2e/test_localstack_inference_monitor.py`, conftest fixtures for KMS/S3/image builds. Deploys stack → seeds data → executes SFN → verifies 5 DynamoDB outcome rows. No AWS credentials needed. First run ~5min; subsequent runs ~30s cached. See `cdk/README.md` LocalStack section.
